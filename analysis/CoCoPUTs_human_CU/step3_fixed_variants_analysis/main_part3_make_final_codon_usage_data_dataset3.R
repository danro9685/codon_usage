# load data
load("data/dataset3/annotations.RData")
load("results/dataset3/codon_usage_information.RData")
load("data/dataset3/human_cu.RData")
load("data/dataset3/sars_cov2_fasta.RData")

# We consider two periods of 12 months the first half and 11 months the second half, starting 2020/01/01

# DATA 1: consider only unique variants for first half of timeline
codon_usage_information_selected = codon_usage_information[which(as.Date(paste0(as.character(substr(codon_usage_information[,"DATE"],1,7)),"-01"),format="%Y-%m-%d")<=as.Date("2020-12-31",format="%Y-%m-%d")),]
...
dataset3_unique_variants_first_half = data.frame(VARIANT_ID=selected_variants,RCU=RCU_val,VF=VF,VARIANT=VARIANT,VARIANT_REDUCED=VARIANT_REDUCED)
...
reduced_variants[which(reduced_variants=="T>G")] = "A>C/T>G"
VARIANT_REDUCED = reduced_variants
dataset3_unique_variants_second_half = data.frame(VARIANT_ID=selected_variants,RCU=RCU_val,VF=VF,VARIANT=VARIANT,VARIANT_REDUCED=VARIANT_REDUCED)
dataset3_unique_variants_second_half = dataset3_unique_variants_second_half[which(!dataset3_unique_variants_second_half$VARIANT_ID%in%dataset3_unique_variants_first_half$VARIANT_ID),]
rownames(dataset3_unique_variants_second_half) = 1:nrow(dataset3_unique_variants_second_half)

# save results
save(dataset3_unique_variants_first_half,file="final_dataset/dataset3_unique_variants_first_half.RData")
save(dataset3_unique_variants_second_half,file="final_dataset/dataset3_unique_variants_second_half.RData")
