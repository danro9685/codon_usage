...
load("final_dataset/dataset1_all_variants.RData")
...
print(prop.test(x=c(APOBEC_group1,APOBEC_group2),n=c(total_group1,total_group2))) # 0.4144719 vs 0.1112617
...
dataset_mut = NULL
dataset_alt = NULL
for(i in 1:nrow(dataset1_all_variants)) {
    curr_valid = NULL
    if(dataset1_all_variants[i,"PROTEIN_MUT"]==dataset1_all_variants[i,"PROTEIN_ALT1"]) {
        curr_valid = c(curr_valid,as.numeric(dataset1_all_variants[i,"VALUE_ALT1"]))
    }
    if(dataset1_all_variants[i,"PROTEIN_MUT"]==dataset1_all_variants[i,"PROTEIN_ALT2"]) {
        curr_valid = c(curr_valid,as.numeric(dataset1_all_variants[i,"VALUE_ALT2"]))
    }
    if(length(curr_valid)>0) {
        dataset_mut = c(dataset_mut,as.numeric(dataset1_all_variants[i,"VALUE_MUT"]))
        dataset_alt = c(dataset_alt,mean(curr_valid))
    }
    cat(i/nrow(dataset1_all_variants),"\n")
}
...
print(fivenum(dataset_mut)) # 0.05 0.13 0.27 0.42 0.73
print(fivenum(dataset_alt)) # 0.07 0.17 0.21 0.35 0.73
print(t.test(dataset_mut,dataset_alt)) # p-value = 0.1068
