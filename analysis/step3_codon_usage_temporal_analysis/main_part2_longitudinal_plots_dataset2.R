...
for(i in sort(names(table(codon_usage_information[,"DATE"])))) {
    ...
    curr_data = dataset2_variants[which(variants_id_data%in%curr_variants),]
...
ggplot(data,aes(x=DATE,y=RCU,fill=DATE)) + geom_boxplot() + ggtitle("Timeline - All variants (APOBEC) considering VF (Dataset 2)")
data = data.frame(RCU=RCU*VF,VARIANT=VARIANT,DATE=DATE)
data = data[which(!data$VARIANT%in%c("C>T","G>A")),]
ggplot(data,aes(x=DATE,y=RCU,fill=DATE)) + geom_boxplot() + ggtitle("Timeline - All variants (NOT APOBEC) considering VF (Dataset 2)")
...
print(mk.test(mean_rcu,alternative="less")$p.value) # pvalue = 0.8038428
...
print(mk.test(mean_rcu,alternative="greater")$p.value) # pvalue = 0.009437773
